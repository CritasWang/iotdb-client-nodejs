...
import { Session, SessionDataSet } from '../src/index';
...
describe('SessionDataSet E2E Tests', () => {
  ...
  test('Should iterate through query results using SessionDataSet', async () => {
    ...
    // Setup test data
    try {
      await session.executeNonQueryStatement('DELETE DATABASE root.dataset_test');
    } catch (e) {
      // Ignore if doesn't exist
    }

    await session.executeNonQueryStatement('CREATE DATABASE root.dataset_test');
    await session.executeNonQueryStatement(
      'CREATE TIMESERIES root.dataset_test.d1.s1 WITH DATATYPE=INT32, ENCODING=PLAIN'
    );
    await session.executeNonQueryStatement(
      'CREATE TIMESERIES root.dataset_test.d1.s2 WITH DATATYPE=TEXT, ENCODING=PLAIN'
    );
    ...
    for (let i = 0; i < 10; i++) {
      await session.executeNonQueryStatement(
        `INSERT INTO root.dataset_test.d1(timestamp, s1, s2) VALUES(${now + i}, ${i}, 'value${i}')`
      );
    }
  ...
  test('Should handle large result sets with lazy loading', async () => {
    ...
    // Setup test data
    try {
      await session.executeNonQueryStatement('DELETE DATABASE root.large_test');
    } catch (e) {
      // Ignore
    }

    await session.executeNonQueryStatement('CREATE DATABASE root.large_test');
    await session.executeNonQueryStatement(
      'CREATE TIMESERIES root.large_test.d1.value WITH DATATYPE=INT32, ENCODING=PLAIN'
    );
    ...
    for (let i = 0; i < 100; i++) {
      await session.executeNonQueryStatement(
        `INSERT INTO root.large_test.d1(timestamp, value) VALUES(${now + i}, ${i})`
      );
    }
  ...
  test('Should support column access by name and index', async () => {
    ...
    // Setup test data
    try {
      await session.executeNonQueryStatement('DELETE DATABASE root.column_test');
    } catch (e) {
      // Ignore
    }

    await session.executeNonQueryStatement('CREATE DATABASE root.column_test');
    await session.executeNonQueryStatement(
      'CREATE TIMESERIES root.column_test.d1.temperature WITH DATATYPE=FLOAT, ENCODING=PLAIN'
    );
    await session.executeNonQueryStatement(
      'CREATE TIMESERIES root.column_test.d1.humidity WITH DATATYPE=DOUBLE, ENCODING=PLAIN'
    );
  ...
  test('Should handle null values correctly', async () => {
    ...
    // Setup test data
    try {
      await session.executeNonQueryStatement('DELETE DATABASE root.null_test');
    } catch (e) {
      // Ignore
    }

    await session.executeNonQueryStatement('CREATE DATABASE root.null_test');
    await session.executeNonQueryStatement(
      'CREATE TIMESERIES root.null_test.d1.s1 WITH DATATYPE=INT32, ENCODING=PLAIN'
    );
    await session.executeNonQueryStatement(
      'CREATE TIMESERIES root.null_test.d1.s2 WITH DATATYPE=INT32, ENCODING=PLAIN'
    );
  ...
  test('Should support toArray() for backward compatibility', async () => {
    ...
    // Setup test data
    try {
      await session.executeNonQueryStatement('DELETE DATABASE root.array_test');
    } catch (e) {
      // Ignore
    }

    await session.executeNonQueryStatement('CREATE DATABASE root.array_test');
    await session.executeNonQueryStatement(
      'CREATE TIMESERIES root.array_test.d1.value WITH DATATYPE=INT32, ENCODING=PLAIN'
    );

    const now = Date.now();
    for (let i = 0; i < 5; i++) {
      await session.executeNonQueryStatement(
        `INSERT INTO root.array_test.d1(timestamp, value) VALUES(${now + i}, ${i})`
      );
    }
